...
class PDFCV(FPDF):
    ...
    def generate_cv_method(self, name, email, phone_number, address, skills, education, work_experience, about_me):
        self.add_page()
        self.ln(20)
...
def generate_cv_pdf():
    ...
    if not name or not email or not phone_number or not address or not skills or not education or not work_experience or not about_me:
        messagebox.showerror("Please fill in all the details!")
        return
    
    cv = PDFCV()
    cv.generate_cv_method(name, email, phone_number, address, skills, education, work_experience, about_me)
